...
public class VkontakteRunnable implements Runnable {
    ...
    public VkontakteRunnable(BeacmcAuth plugin, ServerPlayer player, ProtectedPlayer protectedPlayer) {
        ...
        title = config.getMessage("vkontakte-confirmation-title");
        subtitle = config.getMessage("vkontakte-confirmation-subtitle");
        message = config.getMessage("vkontakte-confirmation-chat");
